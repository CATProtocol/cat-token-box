...
import { SupportedNetwork } from "./configService";
...
export const checkDisableOpCode = function (scriptPubKey: btc.Script) {
...
export function getGuardsP2TR(guardType: GuardType = GuardType.Transfer): {
  ...
  if (guardType === GuardType.Transfer) {
    ...
    throw new Error("unknow guard type");
  }
}
...
export function getCollectionContractP2TR(minterP2TR: string) {
...
export function getDummySigner(
  ...
  if (window.dummySigner === undefined) {
    window.dummySigner = new TestWallet(
  ...
  if (privateKey !== undefined) {
    window.dummySigner.addPrivateKey(privateKey);
  }
  return window.dummySigner;
}
