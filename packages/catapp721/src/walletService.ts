...
import { CollectionInfo, TokenMetadata } from "./metadata";
import * as bitcoinjs from "bitcoinjs-lib";
export const DUMMY_SIG =
...
export class WalletService {
  ...
  async signFeeInput(tx: btc.Transaction) {
    ...
    for (let i = 0; i < psbt.inputCount; i++) {
      if (tx.inputs[i].output.script.isTaproot()) {
        ...
        const pkh = tx.inputs[i].output.script
          .getPublicKeyHash()
          .toString("hex");
        if (pkh === xpubkey) {
          const witnessUtxo = {
      ...
      if (output.script.isTaproot()) {
        ...
        if (pkh === publicKeyHash) {
          tx.inputs[i].setWitnesses([
            Buffer.from(DUMMY_SIG, "hex"),
            Buffer.from(DUMMY_PUBKEY, "hex"),
          ]);
        }
      }
    }
  ...
  async signNft(
    ...
    const xpubkeyToken = toXOnlyFromTaproot(metadata.collectionAddr);
    const xpubkeyFee = await this.getXOnlyPublicKey();
    const { cblock: cblockToken, contract } = getCollectionContractP2TR(
      toP2tr(metadata.minterAddr)
    );
        ...
        if (pkh === xpubkeyToken) {
          psbt.updateInput(i, {
            witnessUtxo: {
              value: BigInt(tx.inputs[i].output.satoshis) || 0n,
              script:
                tx.inputs[i].output.script.toBuffer() || btc.Script.empty(),
            },
            tapLeafScript: [
              {
                leafVersion: 192,
