...
export interface StatefulCovenantUtxo {
    utxo: UTXO;
    txoStateHashes: FixedArray<ByteString, typeof STATE_OUTPUT_COUNT_MAX>;
}
...
export abstract class Covenant<StateT = undefined> {
    ...
    bindToUtxo(utxo: Optional<UTXO, 'script'>): this {
        if (utxo.script && this.lockingScript.toHex() !== utxo.script) {
            throw new Error(
                `Different script, can not bind covenant '${this.constructor.name}' to this UTXO: ${JSON.stringify(
                    utxo,
                )}!`,
            );
        }
    ...
    getSubContractCallArg(inputWitness: Buffer[], method: string, argName: string, alias?: string) {
