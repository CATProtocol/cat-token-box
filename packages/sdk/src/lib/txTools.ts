...
import { MAX_OUTPUT } from '../contracts/utils/txUtil'
import { PrevoutsCtx, SHPreimage, SpentScriptsCtx } from '../contracts/utils/sigHashUtils'
import { btc } from './btc'
...
export const getSpentScripts = function (tx: btc.Transaction): FixedArray<ByteString, typeof MAX_OUTPUT> {
    const lst = emptyFixedArray()
    for (let i = 0; i < tx.inputs.length; i++) {
        const input = tx.inputs[i]
...
export function getTxCtx(tx: btc.Transaction, inputIndex: number, scriptBuffer: Buffer): {
