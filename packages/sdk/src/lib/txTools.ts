...
import { ByteString, ContractTransaction, FixedArray, int2ByteString, toByteString, toHex } from 'scrypt-ts';
import { btc } from './btc';
import { PrevoutsCtx, SHPreimage, SpentScriptsCtx } from '../contracts/types';
...
export function splitSighashPreimage(preimage: Buffer) {
    return {
        ...
        nVersion: preimage.subarray(66, 70),
...
export function toSHPreimageObj(preimageParts, e_, eLastByte): SHPreimage {
    return {
        nVersion: toHex(preimageParts.nVersion),
        nLockTime: toHex(preimageParts.nLockTime),
        shaPrevouts: toHex(preimageParts.shaPrevouts),
        shaSpentAmounts: toHex(preimageParts.shaSpentAmounts),
        shaSpentScripts: toHex(preimageParts.shaSpentScripts),
        shaSequences: toHex(preimageParts.shaSequences),
        shaOutputs: toHex(preimageParts.shaOutputs),
        spendType: toHex(preimageParts.spendType),
        inputIndex: toHex(preimageParts.inputNumber),
        tapLeafHash: toHex(preimageParts.tapLeafHash),
        keyVersion: toHex(preimageParts.keyVersion),
        codeSepPos: toHex(preimageParts.codeSepPos),
        e_: toHex(e_),
...
export const getSpentScripts = function (tx: btc.Transaction): FixedArray<ByteString, typeof TX_OUTPUT_COUNT_MAX> {
    const lst = emptyFixedArray();
    for (let i = 0; i < tx.inputs.length; i++) {
        const input = tx.inputs[i];
...
export const getOutpointString = function (tx: btc.Transaction, index: number) {
    const outputBuf = Buffer.alloc(4, 0);
    outputBuf.writeUInt32LE(index);
    return Buffer.from(tx.id, 'hex').reverse().toString('hex') + outputBuf.toString('hex');
};
...
export function unlockingScriptToWitness(script: btc.Script): Buffer[] {
    return script.chunks.map((value) => {
        if (!value.buf) {
            if ((value.opcodenum >= 81 && value.opcodenum <= 96) || value.opcodenum == 79) {
...
export const contractTxToWitness = function (ct: ContractTransaction): Buffer[] {
...
export function getSHPreimage(
    ...
    while (true) {
        ...
        if (lastByte < 127 && e.add(BigInteger.ONE) < curve.n) {
            eLastByte = lastByte;
            break;
        }
        tx.nLockTime += 1;
...
export function getTxCtx(
    ...
    const prevoutsCtx: PrevoutsCtx = {
        ...
        prevTxHash: Buffer.from(tx.inputs[inputIndex].prevTxId.toString('hex'), 'hex').reverse().toString('hex'),
