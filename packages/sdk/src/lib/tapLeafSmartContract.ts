...
export class TapLeafSmartContract {
    ...
    constructor(contract: SmartContract) {
        ...
        this.contractScriptHash = hash160(this.contract.lockingScript.toBuffer().toString('hex'));
    ...
    witnessToContractCallArgs(witness: Buffer[], method: string): Arguments {
        ...
        if (abiEntity.params.length > argsWitness.length) {
            throw new Error(
                `Unexpected number of witness elements, got ${witness.length - 2}, expected at least ${
                    abiEntity.params.length
                }`,
            );
        }
