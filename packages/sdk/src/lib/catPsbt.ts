...
import { PsbtInput, checkForInput } from 'bip174';
import { TapLeafSmartContract } from './tapLeafSmartContract';
import { emptyOutputByteStrings } from './proof';
import { TX_INPUT_COUNT_MAX, TX_OUTPUT_COUNT_MAX, STATE_OUTPUT_COUNT_MAX } from '../contracts/constants';
import { TxOut, PrevoutsCtx, SHPreimage, SpentScriptsCtx, StateHashes } from '../contracts/types';
...
export type UnlockArgsBuilder = (curPsbt: CatPsbt, tapLeafContract: TapLeafSmartContract) => any[];
...
export class CatPsbt extends Psbt {
    ...
    constructor(txStates = ProtocolState.getEmptyState(), opts: PsbtOptsOptional = {}) {
        super(opts);
    ...
    toTxHex(): string {
        return this.isFinalized ? this.extractTransaction(true).toHex() : this.unsignedTx.toHex();
    }
    ...
    addCovenantInput<T>(covenant: Covenant<T>, subContractAlias?: string): this {
    ...
    updateCovenantInput<T>(inputIndex: number, covenant: Covenant<T>, subContractCall: SubContractCall): this {
        const tapLeafContract = covenant.getTapLeafContract(subContractCall.contractAlias);

        const tapLeafWitness: Witness = [tapLeafContract.contractScript.toBuffer(), tapLeafContract.controlBlock];
        ...
        const witness: Witness = [...contractCallWitness, ...tapLeafWitness];
        ...
        const asyncFinalizer: AsyncFinalizer = async (
            ...
            const contractTx = await subContract.methods[subContractCall.method](...args, {
    ...
    addFeeInputs(feeUtxos: UTXO[]): this {
        for (const utxo of feeUtxos) {
            ...
            this._addSigRequest(index, { address: utxo.address ? script2Addr(script) : utxo.address });
        }
        return this;
    }

    addCovenantOutput<T>(covenant: Covenant<T>, satoshis: number = 330) {
        this.addOutput({
    ...
    getOutputScriptList(): FixedArray<ByteString, typeof STATE_OUTPUT_COUNT_MAX> {
    ...
    getOutputSatoshisList(): FixedArray<ByteString, typeof STATE_OUTPUT_COUNT_MAX> {
        return emptyOutputByteStrings().map((emtpyStr, i) => {
            if (this.txOutputs[i + 1]) {
    ...
    getTxStatesInfo(): TxStateHash {
        return {
            hashRoot: this._txState.hashRoot,
            stateHashes: this._txState.stateHashList,
        };
    }
        ...
        while (true) {
            ...
            if (
                eBuffList.every((eBuff) => {
                    ...
                    return lastByte < 127 && BigInt('0x' + eBuff.toString('hex')) + 1n < N;
                })
            ) {
                found = true;
                break;
    ...
    calculateInputCtxs(): Map<InputIndex, InputContext> {
        ...
        return inputTapLeafHashes.reduce((result, { inputIndex }, index) => {
            ...
            const prevoutsCtx: PrevoutsCtx = {
                ...
                prevOutputIndexVal: BigInt(tx.inputs[inputIndex].outputIndex),
                prevTxHash: Buffer.from(tx.inputs[inputIndex].prevTxId.toString('hex'), 'hex')
                    .reverse()
                    .toString('hex'),
                prevOutputIndex: outputBuf.toString('hex'),
            };
    ...
    getSig(inputIndex: InputIndex, options: Omit<ToSignInput, 'index'>): Sig {
        ...
        if (input.tapScriptSig) {
            ...
            if (tsSig) {
                signature = new Uint8Array(tsSig.signature);
            }
        }
        ...
        if (input.partialSig) {
            ...
            if (pSig) {
                signature = new Uint8Array(Buffer.from(pSig.signature));
            }
        }
    ...
    private _checkInputCnt() {
        ...
        if (inputCnt > TX_INPUT_COUNT_MAX) {
            throw new Error(`This CatPsbt has ${inputCnt} inputs which exceeds the limit of ${TX_INPUT_COUNT_MAX}`);
        }
    }
    ...
    private _checkOutputCnt() {
        ...
        if (outputCnt > TX_OUTPUT_COUNT_MAX) {
            throw new Error(`This CatPsbt has ${outputCnt} outputs which exceeds the limit of ${TX_OUTPUT_COUNT_MAX}`);
        }
    }
