...
import { CAT20Covenant, OpenMinterCat20Meta, toTokenAddress, CAT20Utxo, singleSend } from '@cat-protocol/cat-sdk-v2';
...
export async function singleSendToken(
    ...
    const address = await testErrorSigner.getAddress();
...
describe('Test the feature `send` for `Cat20Covenant`', () => {
    ...
    describe('When sending tokens in a single tx', () => {
        it('should failed send one token utxo use incorrect signer', async () => {
            ...
            return expect(
                testSendResult(tokenUtxos, toReceiverAmount, total - toReceiverAmount),
            ).to.eventually.be.rejectedWith('signature check failed');
        });

        it('should failed multiple token utxos use incorrect signer', async () => {
            const toReceiverAmount = BigInt(metadata.decimals);
