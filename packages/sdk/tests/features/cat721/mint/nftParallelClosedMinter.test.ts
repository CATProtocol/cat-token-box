...
import { NftParallelClosedMinterProto } from '../../../../src/contracts/nft/nftParallelClosedMinterProto'
...
describe('Test the feature `mint` for `NftParallelClosedMinterCovenant`', () => {
    ...
    async function testMintResult(
        inputMinter: NftParallelClosedMinterCovenant,
        spentMinterOutputIndex: number
    ) {
        ...
        const { mintTx, estMintTxVSize } = await mintNft(
            cat721MinterUtxo,
            collectionId,
            metadata
        )
        ...
        const nextLocalIds = [
            nextLocalId + nextLocalId + 1n,
            nextLocalId + nextLocalId + 2n,
        ]
        let nftOutputIndex = 1
        nextLocalIds.forEach((value, index) => {
            if (value < contract.max) {
                nftOutputIndex += 1
                const nextMinter = new NftParallelClosedMinterCovenant(
                    nftReceiverAddr,
                    collectionId,
                    metadata,
                    NftParallelClosedMinterProto.create(
                        inputMinter.nftScript,
                        value
                    )
                )
                expect(
                    Buffer.from(mintTx.txOutputs[index + 1].script).toString(
                        'hex'
                    )
                ).to.eq(nextMinter.lockingScript.toHex())
                expect(
                    mintTx.txState.stateHashList[index],
                    `incorrect minter state on outputs[${index + 1}]`
        ...
        const mintedToken = new CAT721Covenant(
            inputMinter.address,
            CAT721Proto.create(nftReceiverAddr, inputMinter.state!.nextLocalId)
        )
        expect(
            Buffer.from(mintTx.txOutputs[nftOutputIndex].script).toString('hex')
        ).to.eq(mintedToken.lockingScript.toHex())
        expect(mintTx.txState.stateHashList[nftOutputIndex - 1]).eq(
            mintedToken.stateHash
        )
