...
import { ALLOWED_SIZE_DIFF, deployNft, FEE_RATE, mintNft, singleSendNft } from '../nftParallelClosedMinter.utils';
...
describe('Test the feature `send` for `CAT721Covenant`', () => {
    ...
    before(async () => {
        ...
        const { revealTx, collectionId: deployedTokenId, minterAddr: deployedMinterAddr } = await deployNft(metadata);
        ...
        const { mintTx } = await mintNft(cat721MinterUtxo1, collectionId, metadata);
        ...
        const { mintTx: _secondTx } = await mintNft(cat721MinterUtxo2, collectionId, metadata);
        firstMintTx = mintTx;
            ...
            await testSendResult([
                {
                    ...
                    state: CAT721Proto.create(0n, toReceiverAddr),
                },
            ]);
        });
        ...
        it('should send multiple nft utxos successfully', async () => {
            await testSendResult([
                ...
                // second nft utxo
                {
                    ...
                    state: CAT721Proto.create(1n, toReceiverAddr),
                },
            ]);
        });
    });
    ...
    async function testSendResult(cat721Utxos: Cat721Utxo[]) {
        const { guardTx, sendTx, estGuardTxVSize, estSendTxVSize } = await singleSendNft(
            minterAddr,
            cat721Utxos,
        ...
        const toReceiverToken = new CAT721Covenant(
            minterAddr,
            CAT721Proto.create(cat721Utxos[0].state.localId, toReceiverAddr),
        );
