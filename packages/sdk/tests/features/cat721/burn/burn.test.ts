...
import { ALLOWED_SIZE_DIFF, burnNft, deployNft, FEE_RATE, mintNft } from '../nftParallelClosedMinter.utils';
...
describe('Test the feature `burn` for `CAT721Covenant`', () => {
    ...
    before(async () => {
        ...
        const { mintTx } = await mintNft(cat721MinterUtxo, collectionId, metadata);
    ...
    async function testBurnResult(cat721Utxos: Cat721Utxo[]) {
        const { guardTx, burnTx, estGuardTxVSize, estSendTxVSize } = await burnNft(minterAddr, cat721Utxos);
