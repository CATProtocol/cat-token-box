...
// export function getDummyUTXO(satoshis: number = inputSatoshis, unique = false): UTXO {
//     if (unique) {
...
//     const txFee = new btc.Transaction().from(utxos).to(address, 10000).change(address).feePerByte(2).sign(seckey);
...
// export const flags = btc.Script.Interpreter.SCRIPT_VERIFY_WITNESS | btc.Script.Interpreter.SCRIPT_VERIFY_TAPROOT;

// export function verifyInputSpent(psbt: bitcoinjs.Psbt, inputIndex: number): string | true {
...
//         console.log('interpreter', interpreter.pc);
