...
describe('Test multiple cat20 & cat721 token types transfer/burn in a single txn', async () => {
    ...
    class TestCase {
        ...
        private static async createCat20GuardCovenant(guardState: CAT20GuardConstState) {
            const covenant = new CAT20GuardCovenant(guardState);
        ...
        private static async createCat721GuardCovenant(guardState: CAT721GuardConstState) {
            const covenant = new CAT721GuardCovenant(guardState);
            const psbt = new ExtPsbt()
                .spendUTXO(getDummyUtxo(mainAddress))
                // 1e8 is enough for the next txn's fee
                .addCovenantOutput(covenant, 1e8)
        ...
        async test() {
            ...
            const cat20GuardState = CAT20Guard.createEmptyState();
            const cat721GuardState = CAT721Guard.createEmptyState();
            ...
            const cat20ScriptOutputIndexes: Int32[] = [];
            // let cat721ScriptInputIndexes: Int32[] = [];
            const cat721ScriptOutputIndexes: Int32[] = [];
            if (hasCat20) {
                ...
                const guardCovenant = await TestCase.createCat20GuardCovenant(cat20GuardState);
            ...
            if (hasCat721) {
                ...
                const guardCovenant = await TestCase.createCat721GuardCovenant(cat721GuardState);
                psbt.addCovenantInput(guardCovenant);
            }
                    ...
                    invokeMethod: (contract: CAT20Guard, curPsbt: ExtPsbt) => {
                        const ownerAddrOrScripts = fill(toByteString(''), STATE_OUTPUT_COUNT_MAX);
                        {
                            ...
                            const cat20OwnerAddrs = this.cat20s
                                ...
                                .map(({ cat20 }) => cat20.utxo.token.state.ownerAddr);
            ...
            if (hasCat721) {
                this.cat721s.forEach(({ cat721 }, index) => {
                    ...
                    invokeMethod: (contract: CAT721Guard, curPsbt: ExtPsbt) => {
                        const ownerAddrOrScripts = fill(toByteString(''), STATE_OUTPUT_COUNT_MAX);
                        {
                            ...
                            const cat721OwnerAddrs = this.cat721s
                                ...
                                .map(({ cat721 }) => cat721.utxo.nft.state.ownerAddr);
                        ...
                        {
                            const cat721OutputLocalIds = this.cat721s
                                .filter(({ isBurn }) => !isBurn)
                                .map(({ cat721 }) => cat721.utxo.nft.state.localId);
                        ...
                        const cat721States = fill(
                            { ownerAddr: toByteString(''), amount: 0n, localId: 0n },
                            TX_INPUT_COUNT_MAX,
                        );
                        {
