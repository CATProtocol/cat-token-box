...
import { PubKey, ExtPsbt, getBackTraceInfo_, fill, toByteString, STATE_OUTPUT_COUNT_MAX, uint8ArrayToHex, TX_INPUT_COUNT_MAX } from "@scrypt-inc/scrypt-ts-btc";
...
describe('Test incorrect amount for cat20', () => {
    ...
    describe('When output amount is less than the input amount', async () => {
        it('should fail on transfer: input=1000, output=999, burn=0', async () => {
            const cat20 = await createCat20([1000n], mainAddress, 'test')
            return expect(testCase(cat20, [999n], [])).to.eventually.be.rejectedWith('Failed to finalize input 1: Execution failed, Error: Execution failed');
        })
        ...
        it('should fail on both transfer and burn: input=1000, output=500, burn=499', async () => {
            const cat20 = await createCat20([1000n], mainAddress, 'test')
            return expect(testCase(cat20, [500n], [499n])).to.eventually.be.rejectedWith('Failed to finalize input 1: Execution failed, Error: Execution failed');
        })
    });
    ...
    describe('When output amount is greater than the input amount', async () => {
        it('should fail on transfer: input=1000, output=1001, burn=0', async () => {
            const cat20 = await createCat20([1000n], mainAddress, 'test')
            return expect(testCase(cat20, [1001n], [])).to.eventually.be.rejectedWith('Failed to finalize input 1: Execution failed, Error: Execution failed');
        })
        ...
        it('should fail on both transfer and burn: input=1000, output=500, burn=501', async () => {
            const cat20 = await createCat20([1000n], mainAddress, 'test')
            return expect(testCase(cat20, [500n], [501n])).to.eventually.be.rejectedWith('Failed to finalize input 1: Execution failed, Error: Execution failed');
        })
    });
    ...
    async function testCase(
        ...
        const outputStates: CAT20State[] = outputAmountList.filter(amount => amount > 0n).map(amount => ({
        ...
        let guardCovenant = new CAT20GuardCovenant(guardState);
        {
        ...
        let guardInputIndex = cat20.tracedUtxos.length;
        let psbt = new ExtPsbt();
        cat20.tracedUtxos.forEach((utxo, i) => {
        ...
        outputStates.forEach((state) => {
            psbt.addCovenantOutput(new CAT20Covenant(cat20.generater.deployInfo.minterAddr, state), Postage.TOKEN_POSTAGE);
        })
        psbt.change(mainAddress, 0)

        let outputHasCat20 = outputStates.length > 0;
