...
import { getLegacyKeyInfoFromWif, getPrivKey } from './utils/privateKey'
import {
    GetTokenScript,
    closedMinterCall,
    closedMinterDeploy,
...
// keyInfo
const keyInfo = getLegacyKeyInfoFromWif(getPrivKey())
...
describe('Test SmartContract with legacy address', () => {
    ...
    it('should failed mint token with increase use legacy address.', async () => {
        // tx call
        // token state
    ...
    it('should failed mint token without increase use legacy address.', async () => {
        // tx call
        // token state
        const tokenState = CAT20Proto.create(100n, hash160(toByteString('00')))
