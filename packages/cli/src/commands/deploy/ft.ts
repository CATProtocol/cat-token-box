...
function getMinter(wallet: WalletService, genesisId: string, tokenInfo: TokenInfo) {
    ...
    const premineAddress = scaledTokenInfo.premine > 0n ? wallet.getTokenAddress() : toByteString('');
    return getOpenMinterContractP2TR(genesisId, scaledTokenInfo.max, scaledTokenInfo.premine, scaledTokenInfo.limit, premineAddress);
}

export function getMinterInitialTxState(tokenP2TR: string, tokenInfo: TokenInfo): {
...
const buildRevealTx = (wallet: WalletService, genesisId: string, lockingScript: btc.Script, minterType: MinterType, info: TokenInfo, commitTx: btc.Transaction, feeRate: number): btc.Transaction => {

    const { p2tr: minterP2TR } = getMinter(wallet, outpoint2ByteString(genesisId), info);
    ...
    const { sighash } = getSHPreimage(
        revealTx,
        0,
        Buffer.from(tapScript, 'hex'),
    );

    const sig = btc.crypto.Schnorr.sign(wallet.getTaprootPrivateKey(), sighash.hash)
...
export async function deploy(
    ...
    const commitTx = new btc.Transaction()
        ...
        .addOutput(   /** utxo to pay revealTx fee */
            new btc.Transaction.Output({
                satoshis: 0,
                script: changeScript,
            }),
    ...
    const revealTxDummy = buildRevealTx(wallet, dummyGenesisId, lockingScript, minterType, info, commitTx, feeRate);
    ...
    const revealTx = buildRevealTx(wallet, genesisId, lockingScript, minterType, info, commitTx, feeRate);
    ...
    const commitTxId =
        await broadcast(config, wallet, commitTx.uncheckedSerialize());
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const revealTxId =
        await broadcast(config, wallet, revealTx.uncheckedSerialize());
